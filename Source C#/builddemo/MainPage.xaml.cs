...
{    
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async static void UploadChannel()
        {
            ...
            ch.Add("channelUri", JsonValue.CreateStringValue(channel.Uri));
            ch.Add("installationId", JsonValue.CreateStringValue(installationId));

            await App.MobileService.GetTable("channels").InsertAsync(ch);
        }

        private static async System.Threading.Tasks.Task ForceLogin()
        {
            ...
            while (App.MobileService.CurrentUser == null)
            {
                ...
                {
                    // can't await in catch block
                    displayPrompt = true;
                }

                if (displayPrompt)
                {
                    await new Windows.UI.Popups.MessageDialog("Login, fool!", "Login").ShowAsync();
                }
            }
        }
    }
